...
setInterval(function () {
  ...
  client.info("stats" ,function (err, reply) {
    ...
    if (redis_cmd_sec > 1000)
      redis_cmd_sec = numeral(redis_cmd_sec).format('0.0a');
    ...
    if (result_redis_hits_sec > 1000)
      result_redis_hits_sec = numeral(result_redis_hits_sec).format('0.0a');
    ...
    if (result_redis_miss_sec > 1000)
      result_redis_miss_sec = numeral(result_redis_miss_sec).format('0.0a');
    ...
    if (result_redis_expi_sec > 1000)
      result_redis_expi_sec = numeral(result_redis_expi_sec).format('0.0a');
    ...
    if (result_redis_evi_sec > 1000)
      result_redis_evi_sec = numeral(result_redis_evi_sec).format('0.0a');
