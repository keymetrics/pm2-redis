...
setInterval(function () {
  ...
  client.info("stats" ,function (err, reply) {
    ...
    if (redis_cmd_sec > 1000)
      redis_cmd_sec = numeral(redis_cmd_sec).format('0.0 a');
    redisCmdSec.set(redis_cmd_sec);

    var first_result = redis_hits_sec;

    redis_hits_sec = reply.match(/[\n\r].*keyspace_hits:\s*([^\n\r]*)/)[1];

    if (first_result != redis_hits_sec) {
      var last_result = redis_hits_sec;
      var result_redis_hits_sec = last_result - first_result;
    } else {
      var result_redis_hits_sec = 0;
    }

    if (result_redis_hits_sec > 1000)
      result_redis_hits_sec = numeral(result_redis_hits_sec).format('0.0 a');
    ...
    parseInt(redis_miss_sec);
    if (redis_miss_sec > 1000)
      redis_miss_sec = numeral(redis_miss_sec).format('0.0 a');
    redisMissSec.set(redis_miss_sec);

    var redis_expi_sec = reply.match(/[\n\r].*expired_keys:\s*([^\n\r]*)/)[1];
    parseInt(redis_expi_sec);
    if (redis_expi_sec > 1000)
      redis_expi_sec = numeral(redis_expi_sec).format('0.0 a');
    redisExpSec.set(redis_expi_sec);

    var redis_evi_sec = reply.match(/[\n\r].*evicted_keys:\s*([^\n\r]*)/)[1];
    parseInt(redis_evi_sec);
    if (redis_evi_sec > 1000)
      redis_evi_sec = numeral(redis_evi_sec).format('0.0 a');
